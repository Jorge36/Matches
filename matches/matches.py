...
for key, value in dict_result_aux.items():
        list_result = []
        for v in value:
            list_result.append(dict_column2[v[0]])
        dict_result[dict_column1[key]] = list_result

del results
gc.collect()
results = pd.DataFrame.from_dict(dict_result, orient='index')
results.to_csv('/home/jorge/PycharmProjects/matches/results.csv')
